...
def get_data1(rfid):
   # try:
      statement = "SELECT COUNT(*) FROM users WHERE rfid=%s"
      data = (rfid,)
...
def modify_session_time(rfid, WS, session_time):
  try:
    if (WS == 1):
      statement = "UPDATE users SET WS1_time = %s WHERE rfid = %s"
    elif(WS == 2):
      statement = "UPDATE users SET WS2_time = %s WHERE rfid = %s"


    data = (session_time, rfid)
    cursor.execute(statement, data)
    connection.commit()
...
def return_count():
  try:
    ...
    print("The returned count is %d" % cursor.fetchone())
    message = "%s" % cursor.fetchone()
    return (message)
...
num_of_users = return_count()
# print( return_count())
print ("The number of user is %s" % num_of_users)
